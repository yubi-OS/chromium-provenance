...
Linux ARM64 qualification runs on the org's `HIGH-MEM` self-hosted runner (12 CPUs, 62 GiB RAM observed). See [ARM64 CI](docs/arm64-linux-ci.md) for host setup, access restrictions, evidence and native toolchain limitations. Host prerequisites are installed, and [ARM64 qualification CI](https://github.com/yubi-OS/chromium-provenance/actions/runs/36053621319) passes native policy tests plus all 4 verifier tests. Native Chromium toolchain compatibility is the next gate. Full Chromium builds and browser-level provenance enforcement are **not implemented or verified yet**. Existing green CI covers standalone policy logic and verifier scaffolding only.
